...
public class Tooltips {
    ...
    public static void appendTooltip(ItemStack stack, List<Text> tooltip, TooltipContext context) {
        ...
        if (isShulker(stack)) {
            WeightCalculationResult shulkerWeight = BlockWeightCalculator.calculateShulkerBoxWeight(stack);
            handleWeightTooltip(
                    tooltip,
    ...
    private static void handleWeightTooltip(List<Text> tooltip, float totalWeight, float baseWeight, String totalWeightKey, String baseWeightKey, Formatting formatting, int stackCount, ItemStack stack) {
        ...
        if (Screen.hasShiftDown()) {
            ...
            if (stackCount > 1 || (BackpackWeightCalculator.isBackpack(ItemWeights.getItemId(stack),stack) || BackpackWeightCalculator.isTravelerBackpack(stack) || isShulker(stack))) {
